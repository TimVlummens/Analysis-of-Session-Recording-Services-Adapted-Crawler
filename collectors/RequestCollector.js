...
class RequestCollector extends BaseCollector {
    ...
    async addTarget({cdpClient}) {
        ...
        await Promise.all([
            ...
            cdpClient.on('Network.webSocketFrameSent', r => this.handleWebSocketFrame(r))
        ]);
    }
    ...
    /**
     * @param {{initiator: import('../helpers/initiators').RequestInitiator, request: CDPRequest, requestId: RequestId, timestamp: Timestamp, frameId?: FrameId, type?: ResourceType, redirectResponse?: CDPResponse, wallTime: Timestamp, requestPayload: any}} data 
     * @param {import('puppeteer').CDPSession} cdp
     */
    async handleRequest(data, cdp) {
        const {
            requestId: id,
            type,
            request,
        ...
        const hasPostData = request.hasPostData;
        ...
        if (hasPostData && !postData) {
            // try {console.log(await cdp.send('Network.getRequestPostData', {requestId: data.requestId}));} catch(error) {console.log("nothing");}
            try {postData = (await cdp.send('Network.getRequestPostData', {requestId: data.requestId}))["postData"];} catch(error) {}// console.log("nothing");}
        }

        /**
         * @type {InternalRequestData}
         */
        const requestData = {id, url, method, type, initiator, startTime, wallTime, postData, hasPostData};
    ...
    handleWebSocketFrame(request) {
        ...
        this._requests.push({
            ...
            initiator: previousRequest ? previousRequest.initiator : null,
    ...
    getData({urlFilter}) {
        ...
        return this._requests
            ...
            .map(request => ({
                ...
                startTime: request.startTime ? request.startTime : undefined,
...
/**
 * @typedef {'Document'|'Stylesheet'|'Image'|'Media'|'Font'|'Script'|'TextTrack'|'XHR'|'Fetch'|'EventSource'|'WebSocket'|'WebsocketFrame'|'Manifest'|'SignedExchange'|'Ping'|'CSPViolationReport'|'Other'} ResourceType
 */
